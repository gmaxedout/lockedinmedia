...
const App: React.FC = () => {
  ...
  const handleMagicEdit = useCallback(async () => {
    ...
    try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        setError(`Failed to apply the magic edit. ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
  ...
  const TABS: { id: Tab; label: string; icon: React.FC<{ className?: string }> }[] = [
    { id: 'magic', label: 'Magic Edit', icon: MagicWandIcon },
    { id: 'retouch', label: 'Retouch', icon: BullseyeIcon },
  ...
  const renderContent = () => {
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-1 md:gap-2 backdrop-blur-sm">
            {TABS.map(({ id, label, icon: Icon }) => (
                 <button
                    key={id}
                    onClick={() => handleTabChange(id)}
                    className={`w-full flex items-center justify-center gap-2 capitalize font-semibold py-3 px-3 md:px-5 rounded-md transition-all duration-200 text-sm md:text-base ${
                        activeTab === id 
                ...
                >
                    ...
                    <span className="hidden sm:inline">{label === 'Magic Edit' ? 'Magic Edit' : id}</span>
                </button>
            ))}
        </div>
            ...
            {activeTab === 'magic' && (
                <div className="flex flex-col items-center gap-4 animate-fade-in">
                    <p className="text-md text-gray-400 text-center">
                        Describe any change you want to make to the entire image.
                    </p>
                    <form onSubmit={(e) => { e.preventDefault(); handleMagicEdit(); }} className="w-full flex items-stretch gap-2">
                        <textarea
                            value={prompt}
                            onChange={(e) => setPrompt(e.target.value)}
                            placeholder="e.g., 'make the sky a vibrant sunset', 'add a small boat on the lake', 'turn this into a watercolor painting'"
                            className="flex-grow bg-gray-800 border border-gray-700 text-gray-200 rounded-lg p-5 text-lg focus:ring-2 focus:ring-blue-500 focus:outline-none transition w-full disabled:cursor-not-allowed disabled:opacity-60 resize-none"
                            rows={3}
                            disabled={isLoading}
                        />
                        <button 
